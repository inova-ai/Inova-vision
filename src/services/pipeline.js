...
async function renderLocalScene(job, sceneIndex){
  ...
  try {
    ...
    if(voice?.url) await downloadFile(voice.url,voicePath);
    ...
    return {outputUrl:blob.url,voice,renderMode:"local-free"};
  } finally {
...
async function compose(job){
  ...
  try {
    ...
    for(let i=0;i<job.sceneCount;i++){
      ...
      await downloadFile(url,target);sceneFiles.push(target);
    }
...
export async function createPipelineJob({photos,musicFile,productName,style,duration,cta,baseUrl}){
  ...
  const job={
    ...
    storyboard,sceneCount:storyboard.length,musicUrl,musicName:musicFile?.originalname||"",
